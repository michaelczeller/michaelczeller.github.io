...
### Scraping from Wikipedia, wrangling and then joining 
library(rvest)
library(dplyr)
library(stringr)

url <- "https://en.wikipedia.org/wiki/List_of_Shakespearean_settings#Settings_by_scene"
page <- read_html(url)

tables <- page %>% html_nodes("table")

shakespeare_df <- tables[[2]] %>% html_table(fill = TRUE)

shakespeare_df <- shakespeare_df %>%
...
::: {.callout-important collapse="true" title="Enobarbus: Age cannot wither her"}

```{r AgeCannotWitherHer}
#| echo: false
#| message: false
#| warning: false
...
start_row <- aac %>% 
  ...
  filter(
    str_detect(
      PlayerLine,
      fixed("Now Antony must leave her utterly",
            ignore_case = TRUE)
    )
  ) %>%
  pull(row)
...
end_row <- aac %>%
  ...
  filter(
    str_detect(
      PlayerLine,
      fixed("Bless her when she is riggish.",
            ignore_case = TRUE)
    )
  ) %>%
  pull(row)
...
speech_html <- tagList(
  tags$div(
    ...
    tags$h3(
      style = "margin-top:0;",
      glue("Age cannot wither her {citation_label}")
      ),
...
::: {.callout-important collapse="true" title="Gertrude: The lady doth protest too much"}

```{r DothProtestTooMuch}
#| echo: false
#| message: false
#| warning: false
...
speech_html <- tagList(
  tags$div(
    ...
    tags$h3(
      style = "margin-top:0;",
      glue("The lady doth protest too much {citation_label}")
      ),
...
::: {.callout-important collapse="true" title="Lear: Whipping that whore"}
...
speech_html <- tagList(
  tags$div(
    ...
    tags$h3(
      style = "margin-top:0;",
      glue("Whipping that whore {citation_label}")
      ),
