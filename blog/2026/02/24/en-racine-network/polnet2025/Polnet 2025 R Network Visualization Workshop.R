...
##   Polnet 2025 Workshop                                 ##
##   www.kateto.net/polnet2025                            ##
##                                                        ##
...
# Download the workshop materials: https://tinyurl.com/polnet-2025
# Online tutorial: kateto.net/polnet2025
...
# Download an archive with the data files from http://bit.ly/polnet-2025
...
# For example, if the folder was called "polnet2025":
setwd("C:/polnet2025") 
...
# The layout we used earlier, GEM, is also an example of a force-directed algorithm.
...
# 'star', 'circle', 'grid', 'sphere', 'kk', 'fr', 'mds', 'lgl', 'gem', etc.
...
# you can use a minimal or empty theme with theme_minimal() and theme_void() 
...
# This section is a quick reminder that there are other ways to represent 
# a network. For example, we can create a heatmap of a network matrix.
...
visnet_1 <- visNetwork(nodes, links)   # Create the visualization
visSave(visnet_1, "visnetwork_1.html") # Save it in a file
browseURL("visnetwork_1.html")    # Open it in a browser
...
visnet_2 <- visNetwork(nodes, links, height="600px", width="100%", background="#eeefff",
                  main="Network", submain="And what a great network it is!",
                  footer= "Hyperlinks and mentions among media sources")
visSave(visnet_2, "visnetwork_2.html") 
browseURL("visnetwork_2.html")          
...
visnet_3 <- visNetwork(vis.nodes, vis.links)
visSave(visnet_3, "visnetwork_3.html") 
browseURL("visnetwork_3.html")     
...
visnet_4 <- visNetwork(vis.nodes, vis.links)
visSave(visnet_4, "visnetwork_4.html") 
browseURL("visnetwork_4.html")  

# Remove the arrows and set the edge width to 1:
vis.links$arrows <- "" 
vis.links$width  <- 1
visnet_5 <- visNetwork(vis.nodes, vis.links)
visSave(visnet_5, "visnetwork_5.html") 
browseURL("visnetwork_5.html") 

# We can also set the visualization options directly with visNodes() and visEdges()
visnet_6 <- visNetwork(nodes, links)
visnet_6 <- visNodes(visnet_6, shape = "square", shadow = TRUE, 
                    color=list(background="gray", highlight="orange", border="black"))
visnet_6 <- visEdges(visnet_6, color=list(color="black", highlight = "orange"),
                    smooth = FALSE, width=2, dashes= TRUE, arrows = 'middle' ) 
visSave(visnet_6, "visnetwork_6.html") 
browseURL("visnetwork_6.html")
...
visnet_7 <- visOptions(visnet_6, highlightNearest = TRUE, selectedBy = "media") 
visSave(visnet_7, "visnetwork_7.html") 
browseURL("visnetwork_7.html")
...
visnet_8 <- visNetwork(nodes, links)
visnet_8 <- visGroups(visnet_8, groupname = "Newspaper", shape = "square",
                     color = list(background = "gray", border="black"))
visnet_8 <- visGroups(visnet_8, groupname = "TV", shape = "dot",       
                     color = list(background = "tomato", border="black"))
visnet_8 <- visGroups(visnet_8, groupname = "Online", shape = "diamond",   
                     color = list(background = "orange", border="black"))
visnet_8 <- visLegend(visnet_8, main="Legend", position="right", ncol=1) 
visSave(visnet_8, "visnetwork_8.html") 
browseURL("visnetwork_8.html")
...
d3net <-  forceNetwork(Links = links.d3, Nodes = nodes.d3, Source="from", Target="to",
                       NodeID = "idn", Group = "type.label",linkWidth = 1,
                       linkColour = "#afafaf", fontSize=12, zoom=T, legend=T,
                       Nodesize=6, opacity = 1, charge=-600, 
                       width = 600, height = 600)
saveWidget(d3net, file="Media-Network-d3.html")
browseURL("Media-Network-d3.html")                 
